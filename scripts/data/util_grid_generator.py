...
class GridGenerator:
    ...
    def sample_generator(self):
        ...
        """
        index_numbers = np.linspace(
            0, self.number_of_samples - 1, self.number_of_samples, dtype="int"
        )
        ...
        scaled_sobol_sample = qmc.scale(
            sobol_sample, self.lower_bounds, self.upper_bounds
        )
        scaled_sobol_sample = 10**scaled_sobol_sample
        df_sobol_sample = pd.DataFrame(
            data=scaled_sobol_sample, columns=self.parameters, index=indices
        )
    ...
    def modify_params(
        self, g_uv, n_H, cr_zeta, output_prefix, min_Av=1e-6, max_Av=20, n_points=300
    ):
        """
        ...
        with (
            ...
            params_lines[40] = (
                f"{g_uv}\t\t\t !G0 (in Draine field units) -x to +x in 1D\n"
            )
    ...
    def executable_function(self, prefix):
        ...
        self.modify_params(
            g_uv=sample[0], n_H=sample[1], cr_zeta=sample[2], output_prefix=prefix
        )
        print("Calling 3DPDR...")
        call("3DPDR")
