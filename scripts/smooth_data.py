...
def smooth_data(data, iv_index, data_indices, aux_indices):
    ...
    for idx in data_indices:
        data[:, idx] = make_log_smooth_spline(data[:, iv_index], data[:, idx], 1e-4, -30.0, 0.0)
...
if __name__ == "__main__":
    ...
    with h5py.File(file_path, "r") as source_fh:
        with h5py.File(file_path.parent / (f"{file_path.stem}_smooth{file_path.suffix}"), "w") as target_fh:
            ...
            for batch in tqdm(batches):
                ...
                new_data_buffer = Parallel(n_jobs=100)(delayed(smooth_data)(data, iv_index, species_indices, physics_indices) for data in data_buffer)
                # Save all data
